...
class _AICoachingScreenState extends State<AICoachingScreen> {
  ...
  Future<void> _generateMenu(List<String> bodyParts) async {
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyAFVfcWzXDTtc9Rk3Zr5OGRx63FXpMAHqY'),
        headers: {
      ...
      if (response.statusCode == 200) {
        ...
        if (locale != 'ja') {
          AppLogger.info('🌐 非日本語ユーザー検出（$locale）→ 翻訳開始', tag: 'AI_COACHING');
          finalMenu = await _translateMenuToLanguage(text);
          AppLogger.info('✅ 翻訳完了: ${finalMenu.length}文字', tag: 'AI_COACHING');
        } else {
          AppLogger.info('🇯🇵 日本語ユーザー → 翻訳スキップ', tag: 'AI_COACHING');
        }
  ...
  Future<String> _translateMenuToLanguage(String japaneseMenu) async {
    ...
    AppLogger.info('🌐 メニュー翻訳開始: 日本語 → $targetLanguage', tag: 'AI_COACHING');
...
''',
      ...
      if (response.statusCode == 200) {
        ...
        AppLogger.info('✅ 翻訳完了: ${translatedText.length}文字', tag: 'AI_COACHING');
        return translatedText;
      } else {
        throw Exception('Translation API Error: ${response.statusCode}');
      }
    } catch (e) {
      AppLogger.warning('⚠️ 翻訳エラー（日本語メニューを返します）: $e', tag: 'AI_COACHING');
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(AppLocalizations.of(context)!.workout_302d148c, TextStyle(fontSize: 13)),
            Text(AppLocalizations.of(context)!.workout_18419fdb, TextStyle(fontSize: 13)),
            Text(AppLocalizations.of(context)!.workout_995040b8, TextStyle(fontSize: 13)),
            SizedBox(height: 8),
            Text(
              '🏆 Proプランなら月30回！',
              style: TextStyle(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            SizedBox(height: 8),
            Text(AppLocalizations.of(context)!.workout_940a74d8, TextStyle(fontSize: 13)),
            Text(AppLocalizations.of(context)!.workout_d9fd4ff4, TextStyle(fontSize: 13)),
            Text(AppLocalizations.of(context)!.workout_fdf1a277, TextStyle(fontSize: 13)),
          ],
        ),
        actions: [
          TextButton(
